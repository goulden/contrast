/*
Stata code for ‘Association of intravenous contrast with kidney function: a quasi-experimental study using regression discontinuity’
Robert Goulden, Brian H. Rowe, Michal Abrahamowicz, Erin Strumpf, Robyn Tamblyn
JAMA Internal Medicine | Submitted 3 February 2021
*/
...
//eGFR - all values >7 days to 6 months
import delimited "lab_gfr.csv", clear
rename labvalue_gfr gfrOut //as in GFR outcome
rename gfr_time gfrOutTime
drop test_uofm
replace gfrOut=. if gfrOutTime <= (60*24*7)
replace gfrOutTime=. if gfrOutTime <= (60*24*7)
save gfrmulti, replace
...
//CREATE DATASET WITH ALL OUTCOME GFRs (>7 DAYS)
use gfrmulti, clear
merge m:1 studyid using CIN, keep(match) nogen
save CINmulti, replace
use CIN, clear
/
...
drop pvalue N_* m_*
table1_mc_dta2docx using "Tables/eTable1.docx", replace font("Calibri Light",11) datafont("Calibri Light",11) datahalign(left)
use CIN, clear

//Table 1
//Use total column from eTable 1, replace CTPA yes/no with following column of discontinuities
scalar i=0
putexcel set Tables/Table1.xlsx, sheet(Sheet1) replace
foreach v of varlist age male gfr dm htn cad cancer ctas charlson{
	scalar i = i+1
	local j = i
	putexcel A`j' = `"`v'"'
	rdrobust `v' ddimer_c, c(0) h(50 150) p(3) all
	local effect = `e(tau_cl)'
	putexcel B`j' = `"`effect'"'
}

//Table 2
eststo clear
putexcel set Tables/Table2convert.xlsx, sheet(Sheet1) modify
matrix cells = (10,14,17,20)
scalar i=0
...
//Figure 1 (4 panel)
rdplot dm ddimer_c if ddimer_c<150, c(0) nbins(10 20) binselect(es) p(3) graph_options(graphregion(color(white)) legend(off) yscale(r(0 0.5)) ylabel(0(0.1)0.5) title("D. Diabetes",size(med) pos(11)) ytitle("Proportion") scale(1) play(Fig2style) saving(Graphs/dm2,replace)) kernel(triangular)
cd Graphs
graph combine ctpa.gph age.gph gfr.gph dm2.gph, iscale(0.7) imargin(small) graphregion(margin(r=30)) b1("                                D-dimer (ng/ml) relative to cutoff                                             D-dimer (ng/ml) relative to cutoff",size(2.5) justification(left) margin(small))
cd ..

//Figure 2
//local
rdplot lastGFR ddimer_c if ddimer_c>=-50 & ddimer_c<=150, c(0) nbins(10 20) binselect(es) p(1) graph_options(graphregion(color(white)) legend(off) yscale(r(0 100)) ylab(0(20)100) title("A",size(med) pos(11)) xtitle("D-dimer (ng/ml) relative to cutoff") ytitle("Long-term eGFR (mL/min/1.73m{superscript:2})") xline(-8) xline(119) play(Fig2style) saving("Graphs/PrimaryLocal",replace)) kernel(triangular) h(8.049 119.141)
//global
...
//Sensitivity analysis - multiple GFR outcomes
use CINmulti, clear
//using clustered variance
rdrobust gfrOut ddimer_c, c(0) covs(gfrcat* agecat* male dm htn cad cancer ctas charlson) bwselect(msetwo) vce(nncluster studyid) all
rdrobust gfrOut ddimer_c, c(0) fuzzy(ctpa) covs(gfrcat* agecat* male dm htn cad cancer ctas charlson) bwselect(msetwo) vce(nncluster studyid) all
use CIN, clear

//exact size of discontinuity in Figure 1A
rdrobust ctpa ddimer_c, c(0) h(50 150) p(3) all
...
nospace percent onecol total(after) clear
drop N_* m_*
table1_mc_dta2docx using "Tables/eTable2.docx", replace font("Calibri Light",11) datafont("Calibri Light",11) datahalign(left)
use CIN, clear
...
esttab using "Tables/eTable3.csv", ci nostar replace nogaps nolines wide plain
...
esttab using "Tables/eTable4.csv", ci nostar replace nogaps nolines wide plain
